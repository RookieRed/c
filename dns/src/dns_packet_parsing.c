...
int build_dns_request(char * message, char * name) {
    ...
    memset(message, 0, QUESTION_INDEX + name_len);
    ...
    //Mise en place du QDCOUNT à 1
    message[QDCOUNT] = 1;

    //Copiue de la question dans la requete
    memcpy(message + QUESTION_INDEX, name, name_len);
    index = QUESTION_INDEX + name_len;
    message[index++] = 0;
